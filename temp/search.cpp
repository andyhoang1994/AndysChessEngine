...
void getBestMove(Thread* threads, Position* position, Limits* limits, Move* best, Move* ponder) {
    ...
    SearchInfo info = { 0 };
    std::thread pthreads[threads->nthreads];
...
void aspirationWindow(Thread* thread) {
    ...
    while(1) {
        ...
        if(value > alpha && value < beta) {
            ...
            thread->ponderMoves[multiPV] = pv->length > 1 ? pv->line[1] : NON_MOVE;
            return;
        }
...
int search(Thread* thread, PVariation* pv, int alpha, int beta, int depth) {
    ...
    uint16_t move, ttMove = NON_MOVE, bestMove = NON_MOVE;
    ...
    if((tbresult = tablebasesProbeWDL(board, depth, thread->height)) != TB_RESULT_FAILED) {
        ...
        if(ttBound == BOUND_EXACT
            ...
            storeTTEntry(board->hash, NON_MOVE, valueToTT(value, thread->height), VALUE_NONE, depth, ttBound);
            return value;
        }
    }
    ...
    thread->killers[thread->height + 1][0] = NON_MOVE;
    thread->killers[thread->height + 1][1] = NON_MOVE;
    ...
    if(!PvNode
        ...
        while((move = selectNextMove(&movePicker, board, 1)) != NON_MOVE) {
    ...
    while((move = selectNextMove(&movePicker, board, skipQuiets)) != NON_MOVE) {
        ...
        if(isQuiet && depth > 2 && played > 1) {
            ...
            R += inCheck && pieceType(board->squares[getTo(move)]) == KING;
